/* eslint-env jest */
import { errorRange, errorHitType, errorStrength, errorStrengthType, errorToughness, errorToughnessType, errorSave, errorSaveType, errorAP, errorAPType, errorDamage, errorDamageType } from '../src/util/error'
...
test('Save + invulnerable Safe Error', () => {
  expect(() => { errorSave() }).toThrow('Values are not in range, use a value between 2 and 6.')
})

test('Test Save + invulnerable Safe Value Type', () => {
  expect(() => { errorSaveType() }).toThrow('Please use a value between 2 and 6.')
})

test('Test AP Error', () => {
  expect(() => { errorAP() }).toThrow('Values are not in range, use a value between 0 and 6.')
})

test('Test AP Type', () => {
  expect(() => { errorAPType() }).toThrow('Please use a value between 0 and 6.')
})
test('Test DMG Error', () => {
  expect(() => { errorDamage() }).toThrow('Values are not in range, use a value between 1 and 6.')
})

test('Test DMG Type', () => {
  expect(() => { errorDamageType() }).toThrow('Please use a value between 1 and 6.')
})
